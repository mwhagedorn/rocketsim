...
describe PocketRocket::WraspParser do
  describe "when opening the files" do
      let(:parser){
        PocketRocket::EngineParser.new("../lib/pocket_rocket/engines/Estes_A8.eng")
      }
      it "pulls all the data out of the file" do
        ...
        parser.thrust_curve.count.must_equal 23
        parser.thrust_curve.keys.sort.first.must_equal 0.0
        parser.thrust_curve.keys.sort.last.must_equal 0.703
        parser.thrust_curve[0.0].must_equal 0.0
        parser.burn_time.must_equal 0.703
        parser.mass_curve.count.must_equal 23
