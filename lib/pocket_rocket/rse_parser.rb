...
module PocketRocket
  class RseParser
    ...
    def parse
      ...
      context.propellant_weight = doc.xpath('string(//engine/@propWt)').to_f*0.001
      context.engine_weight =doc.xpath('string(//engine/@initWt)').to_f*0.001
      ...
      doc.xpath('//eng-data').each do |node|
        ...
        mass = (context.engine_weight.to_f - node.attribute('m').value.to_f)/1000
